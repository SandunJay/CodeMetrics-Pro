...
public class Cnc {
    ...
    public static void calcCnc(Line lineObj, String line) {
        ...
        try {
            ...
            if (FileHandlerService.stack.peek() != 0 && (line.startsWith("}") || line.endsWith("}"))) {
                String val = FileHandlerService.stack.pop();
            }

            cnc = cnc + FileHandlerService.stack.peek();
            lineObj.setCnc(cnc);
