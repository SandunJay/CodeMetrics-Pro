...
const ProjectData = (props) => {
    ...
    const [isZipUpload, setIsZipUpload] = useState(false);  // Flag to track if zip API was called
...
const [chartData,setChartData]=useState();
  ...
  useEffect(() => {
    async function fetchProjectData() {
        setLoading(true); 
        try {
            const response = await fetch(`http://localhost:8085/api/v1/project/${id}`);
  ...
  useEffect(() => {
    if (project && project.files && project.files.length > 0) {
      ...
      setTotals({
        ...
        // totalComments: totalMultiLineComments+ totalSingleLineComments,
        cps: totalCps,
        cs: totalCs,
        ci: totalCi,
        cnc: totalCnc,
  ...
  if (!project) {
    return <div>Loading...</div>; // Show loading state while project data is being fetched
  }
  ...
  const handleZipAndUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setIsZipUpload(true); 
        const data = await response.json();
        setProject(data); // Set project data on success
      } else {
        setProject(null); // No data available
